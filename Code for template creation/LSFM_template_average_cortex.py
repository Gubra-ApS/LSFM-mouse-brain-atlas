...
temp = sitk.ReadImage(os.path.join(study_folder_server+r'/nifti',reference_brain_id+'_regi_orig.nii.gz'))
...
for i in range(len(sample_id)):
    ...
    temp = sitk.ReadImage(os.path.join(study_folder_server+r'/nifti',sample_id[i]+'_bspline.nii.gz'))
    temp = sitk.GetArrayFromImage(temp)
    temp[temp<0] = 0
    temp[temp>255] = 255
...
avg = np.pad(avg,((170,10),(1,1),(1,1)),'constant', constant_values=(0, 0))
...
sitk.WriteImage(final_sitk,os.path.join(study_folder_server_avg+r'/nifti','avg_gubra_cortex.nii.gz'))
