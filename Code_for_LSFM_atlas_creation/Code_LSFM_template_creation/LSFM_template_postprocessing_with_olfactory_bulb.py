...
### Smoothing
image = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/avg_gubra_final2.nii.gz'))
smoothed = gaussian_filter(image, sigma = 0.15)
bf.saveNifti(smoothed,os.path.join(study_folder_server_avg,'nifti/avg_gubra_final2_smooth.nii.gz'))


#### Image sharpening
image = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/avg_gubra_final2_smooth.nii.gz'))
for i in range(image.shape[0]):
    temp = image[i,:,:]
    sharp = sf.unsharp_mask(temp, radius=1, amount=1.5, preserve_range=True)
    temp[174:,:] = sharp[174:,:]
    temp[temp<0] = 0
    temp[temp>255] = 255
...
bf.saveNifti(image,os.path.join(study_folder_server_avg,'nifti/avg_gubra_final2_smooth_sharp.nii.gz'))

### GET Full symmetry
image = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/avg_gubra_final2_smooth_sharp.nii.gz'))
...
start1 = 0; stop1 = 200+blend_patting

midline2 = 247
#start2 = 50-blend_patting; stop2 = 185+blend_patting
start2 = 200-blend_patting; stop2 = 335+blend_patting

midline3 = 246
#start3 = 185-blend_patting; stop3 = 491
start3 = 335-blend_patting; stop3 = 646
...
bf.saveNifti(final,os.path.join(study_folder_server_avg,'nifti/final2.nii.gz'))



# Set background of template = 0 and ano volume = bg value
final = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/final2.nii.gz'))
bg_mask = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/gubra_tissue_mask.nii.gz'))
bg_mask = np.pad(bg_mask,((0,0),(145,0),(0,0)),'constant', constant_values=(0, 0))
bg_mask[:,0:180,:]=1
final_touch=final*bg_mask
bf.saveNifti(final_touch, os.path.join(study_folder_server_avg,'nifti/final_touch2.nii.gz'))

#Final symmetry of all volumes : mirror one hemisphere
final_touch = bf.readNifti(os.path.join(study_folder_server_avg,'nifti/final_touch2.nii.gz'))
temp = final_touch[:,:,1:230]
print(temp.shape)
print(final_touch.shape)
temp = np.flip(temp,2)
final_touch[:,:,232:232+temp.shape[2]] = temp
final_touch[final_touch<12]=0
bf.saveNifti(final_touch, os.path.join(study_folder_server_avg,'nifti/final_touch_symm2.nii.gz'))
